...
orderRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    // Only the owner of the order or an admin can cancel
    if (req.user.role === 'admin' || order.user.equals(req.user._id)) {
      if (order.status === 'delivered') {
        return res.status(400).send({ message: 'Delivered orders cannot be cancelled.' });
      }
      ...
      order.rejectReason = req.body.reason || 'Cancelled by user';
      await order.save();
